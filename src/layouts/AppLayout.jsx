...
// ── Logo icon (para sidebar colapsada — verde sobre blanco) ──────────────────
function LogoIcon() {
  return (
    <div
...
// ── Logo full (sidebar abierta — blanco sobre verde) ─────────────────────────
function LogoFull() {
  return (
...
function DesktopSidebar({ visibleNav, currentPage, navigate, profile, onLogout, expanded, onToggle }) {
  return (
    <aside
      className="hidden md:flex flex-col flex-shrink-0 h-screen sticky top-0 transition-all duration-200"
      style={{
    ...
    >
      ...
      >
        {expanded ? <LogoFull /> : <div className="w-8" />}
        <button
          onClick={onToggle}
          className="flex h-7 w-7 items-center justify-center rounded-md text-white/50 hover:bg-white/10 hover:text-white transition flex-shrink-0"
        >
      ...
      {/* Nav */}
      <nav className={`flex-1 overflow-y-auto py-3 space-y-0.5 ${ expanded ? 'px-2' : 'px-1.5' }`}>
        {visibleNav.map(item => (
          <SidebarLink
            key={item.id}
            item={item}
      ...
      >
        {/* Notificaciones */}
        <div className={`mb-2 ${ expanded ? 'px-1' : 'flex justify-center' }`}>
          <NotificationBell profile={profile} dark />
        </div>
        ...
        {expanded ? (
          ...
          <div className="flex flex-col items-center gap-2">
            <div
              ...
              className="flex h-7 w-7 items-center justify-center rounded-full bg-white/15 text-xs font-bold text-white cursor-default"
            >
              {profile?.full_name?.[0]?.toUpperCase() || 'U'}
            </div>
          </div>
    ...
    <div className="flex min-h-screen" style={{ backgroundColor: '#f6f5f0' }}>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          />
          <aside
            className="relative z-10 flex h-full w-64 flex-col shadow-2xl"
            style={{ backgroundColor: '#1c473c' }}
          >
            {/* Header */}

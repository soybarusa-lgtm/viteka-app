...
export default function EditPharmacyModal({ pharmacy, onClose }) {
  const { updatePharmacy } = usePharmacies()
  ...
  useEffect(() => {
    if (pharmacy.province) setSelectedProvince(pharmacy.province)
  }, [pharmacy.province])
    ...
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        <div className="modal-header">
          <h2 className="font-bold text-gray-900">Editar farmacia</h2>
        ...
        <form onSubmit={handleSubmit} className="modal-body space-y-5">
          <section>
            ...
            <div className="space-y-3">
              <div><label className="label">Nombre de la farmacia *</label><input className="input" value={form.pharmacy_name || ''} onChange={e => set('pharmacy_name', e.target.value)} required /></div>
              <div><label className="label">Tipo jurídico *</label>
                <select className="input" value={form.legal_type || 'autonomo'} onChange={e => set('legal_type', e.target.value)}>
          ...
          {showOperativa && (
            <section>
              ...
              <div className="space-y-3">
                <div><label className="label">Horario</label><input className="input" value={form.schedule || ''} onChange={e => set('schedule', e.target.value)} /></div>

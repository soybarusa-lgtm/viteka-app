...
// ---------------------------------------------------------------------------
// Constants (idénticas a CreateClientModal)
// ---------------------------------------------------------------------------
const PROVINCES_AN = ['Almería','Cádiz','Córdoba','Granada','Huelva','Jaén','Málaga','Sevilla']

const CITIES_BY_PROVINCE = {
  'Almería':  ['Almería','Adra','Berja','El Ejido','Huércal-Overa','Níjar','Roquetas de Mar','Vera','Vícar','Otros'],
  'Cádiz':    ['Algeciras','Arcos de la Frontera','Cádiz','Chiclana de la Frontera','El Puerto de Santa María','Jerez de la Frontera','La Línea de la Concepción','Rota','San Fernando','Sanlúcar de Barrameda','Tarifa','Otros'],
  'Córdoba':  ['Baena','Cabra','Córdoba','Lucena','Montilla','Palma del Río','Peñarroya-Pueblonuevo','Pozoblanco','Priego de Córdoba','Puente Genil','Otros'],
  'Granada':  ['Almuñécar','Baza','Granada','Guadix','Huéscar','Loja','Maracena','Motril','Santa Fe','Otros'],
  'Huelva':   ['Almonte','Ayamonte','Huelva','Isla Cristina','Lepe','Moguer','Nerva','Palma del Condado','Valverde del Camino','Otros'],
  'Jaén':     ['Alcalá la Real','Andújar','Baeza','Jaén','Linares','Martos','Úbeda','Villacarrillo','Otros'],
  'Málaga':   ['Álora','Antequera','Benalmádena','Estepona','Fuengirola','Marbella','Málaga','Mijas','Nerja','Rincón de la Victoria','Ronda','Torremolinos','Torre del Mar','Torrox','Vélez-Málaga','Otros'],
  'Sevilla':  ['Alcalá de Guadaíra','Bormujos','Carmona','Dos Hermanas','Écija','Lebrija','Mairena del Aljarafe','Morón de la Frontera','Osuna','Sevilla','Utrera','Otros'],
}
...
// ---------------------------------------------------------------------------
// Step1 — idéntico a CreateClientModal + población desplegable
// ---------------------------------------------------------------------------
function Step1({ data, onChange }) {
  ...
  function setProvince(val) {
    // al cambiar provincia, limpiar ciudad si no está en la nueva lista
    const cities = CITIES_BY_PROVINCE[val] || []
    const city = cities.includes(data.city) ? data.city : ''
    onChange({ ...data, province: val, city })
  }
  ...
  const cities   = CITIES_BY_PROVINCE[data.province] || []
        ...
        <fieldset className="rounded-xl p-4 space-y-4" style={{ border: '1px solid var(--border)' }}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="Provincia">
              <Sel value={data.province||''} onChange={e=>setProvince(e.target.value)}>
                <option value="">Seleccionar...</option>
                {PROVINCES_AN.map(p=><option key={p} value={p}>{p}</option>)}
              </Sel>
            </Field>
            <Field label="Población">
              <Sel value={data.city||''} onChange={e=>set('city',e.target.value)} disabled={!data.province}>
                <option value="">{data.province ? 'Seleccionar...' : 'Selecciona provincia primero'}</option>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {i >= 2 && (
                      <button type="button" onClick={() => removeOwner(i)}
                        className="text-[11px] hover:underline" style={{ color: 'var(--badge-red-text)' }}>Eliminar</button>
                    )}
                  </div>
...
// ---------------------------------------------------------------------------
// Step2 — productos (idéntico a CreateClientModal)
// ---------------------------------------------------------------------------
...
export default function EditClientModal({ isOpen, client, onClose, onSave }) {
  ...
  // Precarga datos del cliente al abrir
  useEffect(() => {
    if (client) {
      const { products: prod, ...rest } = client
      setClientData({
        ...rest,
        legal_type: rest.legal_type || [],
        cb_owners:  rest.cb_owners  || [{ name:'', nif:'', collegiate_number:'' }, { name:'', nif:'', collegiate_number:'' }],
      })
      setProducts(prod || {})
      setStep(1)
    }
  ...
  async function handleSave() {
    setSaving(true)
    try {
      await onSave(client.id, { ...clientData, products })
    } catch (err) {
      alert(err.message)
    } finally {
      setSaving(false)
    ...
    <div className="fixed inset-0 z-[300] flex items-start justify-center overflow-y-auto p-4 pt-8"
      ...
      <div className="relative w-full max-w-2xl rounded-2xl shadow-2xl"
        ...
        <div className="flex items-center justify-between border-b px-6 py-4" style={{ borderColor: 'var(--border)' }}>
          <div>
            <h2 className="text-[16px] font-semibold" style={{ color: 'var(--text)' }}>Editar farmacia</h2>
            {client.pharmacy_name && (
              <p className="mt-0.5 text-[12px]" style={{ color: 'var(--muted)' }}>{client.pharmacy_name}</p>
            )}
          </div>

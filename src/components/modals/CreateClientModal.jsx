...
function validateForm(data) {
  ...
  for (const field of requiredBase) {
    if (!data[field] || String(data[field]).trim() === '') {
      return {
        valid: false,
        message: 'Faltan campos obligatorios de la farmacia.',
      }
    }
  }
  ...
  if (!/\S+@\S+\.\S+/.test(data.contact_email || '')) {
    return {
      valid: false,
      message: 'El email de contacto no tiene formato válido.',
    }
  }
  ...
  if (data.legal_type === 'cb') {
    ...
    for (const field of cbRequired) {
      if (!data[field] || String(data[field]).trim() === '') {
        return {
          valid: false,
          message: 'Faltan datos obligatorios de Comunidad de Bienes (CB).',
        }
      }
    }
  }
  ...
  if (data.legal_type === 'sl') {
    ...
    for (const field of slRequired) {
      if (!data[field] || String(data[field]).trim() === '') {
        return {
          valid: false,
          message: 'Faltan datos obligatorios de Sociedad Limitada (SL).',
        }
      }
    }
  }

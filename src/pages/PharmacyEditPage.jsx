...
// ── Constantes (igual que NewPharmacyPage) ───────────────────────────────────
const PROVINCES = [
      ...
      {showGuardsAndSchedule && (
        <>
          ...
          <div className="flex items-center gap-2 pt-5">
            <input type="checkbox" id={`guards_${data.__key}`}
...
// ── Helpers de mapeo BD → form / form → BD ─────────────────────────────────────
...
function pharmacyToForm(ph, eq) {
  ...
  const types   = lType.split('_') // 'autonomo_sl' → ['autonomo','sl']
  ...
  const autoContact = mkContact('auto', hasAuto ? {
  ...
  const cbContact = mkContact('cb', hasCb ? {
    contact_phone: ph.contact_phone, contact_email: ph.contact_email,
    address: ph.address, province: ph.province, city: ph.city,
    postal_code: ph.postal_code, soe_number: ph.soe_number,
    schedule: ph.schedule, has_guards: ph.has_guards, observations: ph.observations,
  ...
  return {
    ...
    pantallas_interior:   ubs.includes('Interior'),
    pantallas_escaparate: ubs.includes('Escaparate'),
    pantallas_exterior:   ubs.includes('Exterior'),
...
export default function PharmacyEditPage() {
  ...
  const [eqId,    setEqId]    = useState(null)   // id de pharmacy_equipment
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (phErr) { setError(phErr.message); setLoading(false); return }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const { error: phErr } = await supabase
        .from('pharmacies').update(pharmacyPayload).eq('id', id)
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {hasSl && (
          <Section title="Sociedad Limitada (S.L.)" subtitle={hasAuto || hasCb ? 'Datos propios de la S.L.' : 'Datos de la sociedad y contacto'}>
            ...
            <hr className="border-gray-100" />
            <ContactBlock data={form.sl_contact} onChange={(f,v) => setContact('sl_contact',f,v)}
              showGuardsAndSchedule={!hasAuto && !hasCb} showSoe={!hasAuto && !hasCb} />
          </Section>
        )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {CAJA_OPTIONS.map(opt => (
              <ChipBtn key={opt.value} active={form.caja === opt.value}
                onClick={() => { set('caja', opt.value); set('caja_modelo','') }}>{opt.label}</ChipBtn>
            ))}
          </div>
          {form.caja !== 'NO' && (
        ...
        <Section title="Robot dispensador">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            {ROBOT_OPTIONS.map(opt => <ChipBtn key={opt} active={form.robot === opt} onClick={() => set('robot', opt)}>{opt === 'NO' ? 'No tiene' : opt}</ChipBtn>)}
          </div>
          {form.robot !== 'NO' && (
            <div className="grid grid-cols-2 gap-3">
        ...
        <Section title="Cruz luminosa">
          <div className="flex gap-2">
            {CRUZ_OPTIONS.map(opt => <ChipBtn key={opt} active={form.cruz === opt} onClick={() => set('cruz', opt)}>{opt === 'NO' ? 'No tiene' : opt}</ChipBtn>)}
          </div>
          {form.cruz !== 'NO' && (
            <div className="grid grid-cols-2 gap-3">
        ...
        <div className="flex gap-3 justify-end pt-2">
          <button type="button" onClick={() => navigate(-1)}
            className="px-5 py-2 rounded-lg border border-gray-300 text-sm text-gray-600 hover:bg-gray-50">Cancelar</button>

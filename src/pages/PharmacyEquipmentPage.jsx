  ...
  return (
    <div className="flex flex-wrap gap-2 mb-1">
      {options.map(opt => {
        ...
        const active = value === val || (!value && (val === 'no' || val === 'NO'))
        return (
...
function YearInput({ value, onChange, label = 'Año instalación' }) {
  return (
    <Field label={label}>
      <input className="input w-28" type="number" min="1990" max="2099"
        placeholder="Ej: 2022" value={value || ''} onChange={e => onChange(e.target.value)} />
    </Field>
  )
}
  ...
  return (
    <div className="space-y-2 mt-2">
          ...
          {['SI','NO'].map(v => (
            <label key={v} className={`radio-pill${data.viteka_distrib===v?' active':''}`}>
              <input type="radio" className="sr-only"
                checked={data.viteka_distrib===v}
                onChange={() => onChange({ ...data, viteka_distrib: v })} />
              {v}
            </label>
          ))}
        </div>
      ...
      {data.viteka_distrib === 'NO' && (
        <SatisfactionStars value={data.satisfaction}
          onChange={v => onChange({ ...data, satisfaction: v })}
          label="Grado de satisfacción con su distribuidor actual" />
      )}
      <Field label="Observaciones sobre soporte">
        <input className="input" placeholder="Notas…" value={data.soporte_obs||''}
          onChange={e => onChange({ ...data, soporte_obs: e.target.value })} />
      </Field>
    </div>
  )
}
...
function SeccionCaja({ data, onChange }) {
  ...
  return (
    <div className="card">
      ...
      <RadioPills name="caja" options={CAJA_OPTIONS} value={caja}
        onChange={v => { set('caja',v); set('caja_detail',{}) }} />
      ...
      <Conditional show={caja==='otro'}>
        <Field label="Modelo (texto libre)">
          <input className="input" value={cajaData.modelo_otro||''} onChange={e=>set('caja_detail',{...cajaData,modelo_otro:e.target.value})} />
        </Field>
      </Conditional>
...
function SeccionESL({ data, onChange }) {
  ...
  return (
    <div className="card">
      ...
      <RadioPills name="esl" options={ESL_OPTIONS} value={esl}
        onChange={v => { set('esl',v); set('esl_detail',{}) }} />
      <Conditional show={esl==='otro'}>
        <Field label="Indicar marca/modelo">
          <input className="input" value={eslData.otro_nombre||''} onChange={e=>set('esl_detail',{...eslData,otro_nombre:e.target.value})} />
        </Field>
      </Conditional>
...
function SeccionBascula({ data, onChange }) {
  ...
  return (
    <div className="card">
      ...
      <RadioPills name="bascula" options={BASCULA_OPTIONS} value={val}
        onChange={v => { set('bascula',v); set('bascula_detail',{}) }} />
...
function SeccionAntihurto({ data, onChange }) {
  ...
  return (
    <div className="card">
      ...
      <RadioPills name="antihurto" options={ANTIHURTO_OPTIONS} value={val}
        onChange={v => { set('antihurto',v); set('antihurto_detail',{}) }} />
      <Conditional show={val!=='no'}>
        <div className="flex flex-wrap gap-3">
...
function SeccionConsultoria({ data, onChange }) {
  ...
  return (
    <div className="card">
      ...
      <RadioPills name="consultoria" options={CONSULTORIA_OPTIONS} value={val}
        onChange={v => { set('consultoria',v); set('consultoria_detail',{}) }} />
      <Conditional show={val!=='no'}>
        <div className="flex flex-wrap gap-3">
          ...
          <Field label="Año inicio">
            <input className="input w-28" type="number" min="1990" max="2099" placeholder="2022" value={det.anio||''} onChange={e=>set('consultoria_detail',{...det,anio:e.target.value})} />
          </Field>
        </div>
      </Conditional>
    </div>
...
function SeccionRobot({ data, onChange }) {
  ...
  return (
    <div className="card">
      ...
      <RadioPills name="robot" options={ROBOT_OPTIONS} value={val}
        onChange={v => { set('robot',v); set('robot_detail',{}) }} />
      <Conditional show={val!=='NO'}>
        <div className="flex flex-wrap gap-3">
...
// ── Campos detallados IT ──────────────────────────────────────────────────────
function DiskList({ disks=[], onChange }) {
  const add    = () => onChange([...disks, { tipo:'SSD', capacidad:'' }])
  const remove = i  => onChange(disks.filter((_,idx)=>idx!==i))
  const set    = (i,f,v) => onChange(disks.map((d,idx)=>idx===i?{...d,[f]:v}:d))
  return (
    <div>
      ...
      {disks.map((d,i)=>(
        <div key={i} className="flex flex-wrap gap-2 items-end bg-white rounded-lg p-2 border border-gray-200 mb-2">
          <Field label="Tipo"><select className="input w-24" value={d.tipo} onChange={e=>set(i,'tipo',e.target.value)}>{DISK_TYPES.map(t=><option key={t}>{t}</option>)}</select></Field>
          <Field label="Capacidad"><input className="input w-28" placeholder="512 GB" value={d.capacidad} onChange={e=>set(i,'capacidad',e.target.value)} /></Field>
          <button type="button" onClick={()=>remove(i)} className="text-red-400 hover:text-red-600 pb-1 text-lg">×</button>
        </div>
      ))}
      <button type="button" onClick={add} className="btn-add text-xs">+ Añadir disco</button>
    </div>
  )
}

function IpList({ ips=[], onChange }) {
  const add    = () => onChange([...ips, ''])
  const remove = i  => onChange(ips.filter((_,idx)=>idx!==i))
  const set    = (i,v) => onChange(ips.map((ip,idx)=>idx===i?v:ip))
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2">
        {ips.map((ip,i)=>(
          <div key={i} className="flex items-center gap-1">
            <input className="input w-40" placeholder="192.168.1.10" value={ip} onChange={e=>set(i,e.target.value)} />
            <button type="button" onClick={()=>remove(i)} className="text-red-400 hover:text-red-600 text-lg">×</button>
          </div>
        ))}
        <button type="button" onClick={add} className="btn-add text-xs self-end">+ IP</button>
      </div>
    </div>
  )
}
        ...
        {device.monitor==='SI'&&<>
          ...
          <Field label="Conexión">
            <select className="input" value={device.monitor_conn||''} onChange={e=>s('monitor_conn',e.target.value)}>
              <option value="">…</option>
              {MONITOR_CONN.map(c=><option key={c}>{c}</option>)}
            </select>
          </Field>
        </>}
      </div>
...
function SaiList({ items=[], onChange }) {
  const add    = () => onChange([...items, { marca:'',modelo:'',capacidad:'',anio:'',vinculado:'' }])
  const remove = i  => onChange(items.filter((_,idx)=>idx!==i))
  const set    = (i,f,v) => onChange(items.map((x,idx)=>idx===i?{...x,[f]:v}:x))
  return (
    <div className="space-y-2">
      {items.map((item,i)=>(
        <div key={i} className="bg-white rounded-lg border border-gray-200 p-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            <Field label="Marca"><input className="input" value={item.marca} onChange={e=>set(i,'marca',e.target.value)} /></Field>
            <Field label="Modelo"><input className="input" value={item.modelo} onChange={e=>set(i,'modelo',e.target.value)} /></Field>
            <Field label="Capacidad"><input className="input" placeholder="1500 VA" value={item.capacidad} onChange={e=>set(i,'capacidad',e.target.value)} /></Field>
            <Field label="Año"><input className="input" type="number" value={item.anio} onChange={e=>set(i,'anio',e.target.value)} /></Field>
            <Field label="Equipo vinculado"><input className="input" value={item.vinculado} onChange={e=>set(i,'vinculado',e.target.value)} /></Field>
          </div>
        </div>
      ))}
...
function RouterList({ items=[], onChange }) {
  const add    = () => onChange([...items, { marca:'',modelo:'',proveedor:'',anio:'',prioridad:'Principal',contacto:{nombre:'',cargo:'',telefono:'',email:''} }])
  const remove = i  => onChange(items.filter((_,idx)=>idx!==i))
  const set    = (i,f,v) => onChange(items.map((x,idx)=>idx===i?{...x,[f]:v}:x))
  const setC   = (i,f,v) => onChange(items.map((x,idx)=>idx===i?{...x,contacto:{...x.contacto,[f]:v}}:x))
  return (
    <div className="space-y-2">
      {items.map((item,i)=>(
        <div key={i} className="bg-white rounded-lg border border-gray-200 p-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-3">
            <Field label="Marca"><input className="input" value={item.marca} onChange={e=>set(i,'marca',e.target.value)} /></Field>
            <Field label="Modelo"><input className="input" value={item.modelo} onChange={e=>set(i,'modelo',e.target.value)} /></Field>
            <Field label="Proveedor"><input className="input" value={item.proveedor} onChange={e=>set(i,'proveedor',e.target.value)} /></Field>
            <Field label="Año"><input className="input" type="number" value={item.anio} onChange={e=>set(i,'anio',e.target.value)} /></Field>
            <Field label="Prioridad">
              <select className="input" value={item.prioridad} onChange={e=>set(i,'prioridad',e.target.value)}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <Field label="Nombre"><input className="input" value={item.contacto?.nombre||''} onChange={e=>setC(i,'nombre',e.target.value)} /></Field>
            <Field label="Cargo"><input className="input" value={item.contacto?.cargo||''} onChange={e=>setC(i,'cargo',e.target.value)} /></Field>
            <Field label="Teléfono"><input className="input" value={item.contacto?.telefono||''} onChange={e=>setC(i,'telefono',e.target.value)} /></Field>
            <Field label="Email"><input className="input" type="email" value={item.contacto?.email||''} onChange={e=>setC(i,'email',e.target.value)} /></Field>
          </div>
        </div>
      ))}
...
function SwitchList({ items=[], onChange }) {
  const add    = () => onChange([...items, { marca:'',modelo:'',salidas:'',gestionable:'NO',anio:'',capa:'2ª',poe:'NO',puertos_poe:'' }])
  const remove = i  => onChange(items.filter((_,idx)=>idx!==i))
  const set    = (i,f,v) => onChange(items.map((x,idx)=>idx===i?{...x,[f]:v}:x))
  return (
    <div className="space-y-2">
      {items.map((item,i)=>(
        <div key={i} className="bg-white rounded-lg border border-gray-200 p-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <Field label="Marca"><input className="input" value={item.marca} onChange={e=>set(i,'marca',e.target.value)} /></Field>
            <Field label="Modelo"><input className="input" value={item.modelo} onChange={e=>set(i,'modelo',e.target.value)} /></Field>
            <Field label="Nº salidas"><input className="input" type="number" value={item.salidas} onChange={e=>set(i,'salidas',e.target.value)} /></Field>
            <Field label="Año"><input className="input" type="number" value={item.anio} onChange={e=>set(i,'anio',e.target.value)} /></Field>
            <Field label="Gestionable">
              <select className="input" value={item.gestionable} onChange={e=>set(i,'gestionable',e.target.value)}>
                <option>NO</option><option>SI</option>
              </select>
            </Field>
            <Field label="Capa">
              <select className="input" value={item.capa} onChange={e=>set(i,'capa',e.target.value)}>
                {CAPA_OPTIONS.map(c=><option key={c}>{c}</option>)}
              </select>
            </Field>
            <Field label="PoE">
              <select className="input" value={item.poe} onChange={e=>set(i,'poe',e.target.value)}>
                <option>NO</option><option>SI</option>
              </select>
            </Field>
            {item.poe==='SI'&&<Field label="Puertos PoE"><input className="input" type="number" value={item.puertos_poe} onChange={e=>set(i,'puertos_poe',e.target.value)} /></Field>}
          </div>
        </div>
      ))}
...
function DeviceCard({ device, allDevices, onChange, onDelete, onDuplicate }) {
  ...
  function renderTypeFields() {
    if (['Servidor','Estación'].includes(device.tipo))
      return <ComputerFields device={device} onChange={onChange} />
    if (['Impresora documentos','Impresora tickets','Impresora etiquetas adhesivas'].includes(device.tipo))
      return <PrinterFields device={device} onChange={onChange} allDevices={allDevices} />
  ...
  return (
    <div className="modal-backdrop">
      <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4 shadow-xl">
        ...
        <input className="input mb-3" placeholder="Buscar farmacia…"
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button className="btn-primary" disabled={!selected} onClick={()=>onConfirm(selected)}>Copiar equipos</button>
        </div>
      </div>
    </div>
  )
...
// ── SeccionEquiposIT ──────────────────────────────────────────────────────────
function SeccionEquiposIT({ devices, onChange, onSaveDevice, onDuplicate, onDeleteDevice, pharmacyId, copyDevicesFromPharmacy }) {
  ...
  function deleteDevice(i) {
    const d = devices[i]
    if (d?.id) onDeleteDevice(d.id)
    else onChange(devices.filter((_,idx)=>idx!==i))
  }
  ...
  return (
    <div className="card">
      ...
      <p className="text-xs text-gray-500 mb-4">
        Equipos VITEKA: control de garantías y seguimiento. Equipos externos: conocimiento de infraestructura para sustitución.
      </p>
      <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
        <p className="text-sm text-gray-500">{devices.length} equipo{devices.length!==1?'s':''}</p>
        <div className="flex items-center gap-2 flex-wrap">
          <button type="button" onClick={()=>{ setCopyError(''); setShowCopyModal(true) }}
            className="btn-secondary text-xs">📋 Copiar de farmacia</button>
          <button type="button" onClick={()=>setGroupByType(p=>!p)}
            className={`text-xs px-3 py-1.5 rounded-lg border transition font-medium ${
              groupByType?'bg-teal-50 border-teal-300 text-teal-700':'bg-white border-gray-200 text-gray-500 hover:border-gray-300'
            }`}>
...
export default function PharmacyEquipmentPage({ pharmacyId, pharmacyName, navigate }) {
  ...
  async function handleSave() {
    ...
    } catch { /* error ya gestionado en hook */ }
  }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center" style={{ minHeight:'60vh' }}>
        <div style={{ width:32, height:32, border:'4px solid #1c473c',
  ...
  return (
    <div style={{ maxWidth:900, margin:'0 auto', paddingBottom:80 }}>
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={()=>navigate('pharmacy-detail',{pharmacyId})}
      ...
      <SeccionEquiposIT
        ...
        onSaveDevice={async d => { /* guardado diferido al pulsar Guardar */ }}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200 px-4 py-3 flex justify-end gap-3">
        <button type="button" className="btn-secondary"
          onClick={()=>navigate('pharmacy-detail',{pharmacyId})}>Cancelar</button>

...
export default function NewPharmacyPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast(`Farmacia "${form.pharmacy_name}" creada correctamente`, 'success')
      navigate(`/farmacias/${pharmacy.id}`)
    } catch (err) {
      toast(err.message || 'Error al crear la farmacia', 'error')
    } finally {
      setSaving(false)
    }
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Section title="Datos básicos">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <ToggleBtn active={hasAuto} disabled={false} onClick={() => toggleType('autonomo')}>{hasAuto ? '✓ ' : ''}Autónomo</ToggleBtn>
              <ToggleBtn active={hasCb}   disabled={false} onClick={() => toggleType('cb')}>{hasCb ? '✓ ' : ''}C.B.</ToggleBtn>
              <ToggleBtn active={hasSl}   disabled={false} onClick={() => toggleType('sl')}>{hasSl ? '✓ ' : ''}S.L.</ToggleBtn>
            </div>
          </div>
        </Section>
        ...
        {hasAuto && (
          <Section title="Autónomo" subtitle="Datos del titular y contacto de la farmacia">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div><Label>NIF</Label><Input value={form.auto.nif} onChange={e => setNested('auto','nif',e.target.value)} placeholder="00000000X" /></div>
        ...
        {hasCb && (
          <Section title="Comunidad de Bienes (C.B.)" subtitle="Datos de la sociedad, titulares y contacto de la farmacia">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><Label>Nombre razón social</Label><Input value={form.cb.razon_social} onChange={e => setNested('cb','razon_social',e.target.value)} /></div>
        ...
        {hasSl && (
          <Section title="Sociedad Limitada (S.L.)" subtitle={hasAuto || hasCb ? 'Datos propios de la S.L.' : 'Datos de la sociedad y contacto de la farmacia'}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><Label>Nombre razón social</Label><Input value={form.sl.razon_social} onChange={e => setNested('sl','razon_social',e.target.value)} /></div>
        ...
        <Section title="ERP">
          ...
          {!form.erp_viteka && <div><Label>Grado de satisfacción con el ERP actual</Label><SatisfactionSelect value={form.erp_satisfaction} onChange={e => set('erp_satisfaction', e.target.value)} /></div>}
        </Section>
          ...
          {form.caja !== 'NO' && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {cajaOpt?.modelos?.length > 0 && (
                <div><Label>Modelo</Label><Select value={form.caja_modelo} onChange={e => set('caja_modelo', e.target.value)}><option value="">Seleccionar modelo</option>{cajaOpt.modelos.map(m => <option key={m} value={m}>{m}</option>)}</Select></div>
              )}
        ...
        <Section title="Etiquetas electrónicas (ESL)">
          <div className="flex flex-wrap gap-2">{ESL_OPTIONS.map(opt => <ChipBtn key={opt.value} active={form.esl === opt.value} onClick={() => set('esl', opt.value)}>{opt.label}</ChipBtn>)}</div>
          {form.esl !== 'NO' && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div><Label>Año instalación</Label><YearSelect value={form.esl_year} onChange={e => set('esl_year', e.target.value)} /></div><VitekaCheck value={form.esl_viteka} onChange={v => set('esl_viteka', v)} />{!form.esl_viteka && <div className="sm:col-span-2"><Label>Grado de satisfacción</Label><SatisfactionSelect value={form.esl_satisfaction} onChange={e => set('esl_satisfaction', e.target.value)} /></div>}</div>)}
        </Section>

        <Section title="Básculas">
          <div className="flex flex-wrap gap-2">{BASCULA_OPTIONS.map(opt => <ChipBtn key={opt} active={form.bascula === opt} onClick={() => set('bascula', opt)}>{opt === 'NO' ? 'No tiene' : opt}</ChipBtn>)}</div>
          {form.bascula !== 'NO' && (<div className="grid grid-cols-2 gap-3">{form.bascula === 'Otro' && <div><Label>Indicar marca</Label><Input value={form.bascula_otro} onChange={e => set('bascula_otro', e.target.value)} /></div>}<div><Label>Año instalación</Label><YearSelect value={form.bascula_year} onChange={e => set('bascula_year', e.target.value)} /></div><VitekaCheck value={form.bascula_viteka} onChange={v => set('bascula_viteka', v)} /></div>)}
        </Section>

        <Section title="Arcos antihurto">
          <div className="flex flex-wrap gap-2">{ANTIHURTO_OPTIONS.map(opt => <ChipBtn key={opt} active={form.antihurto === opt} onClick={() => set('antihurto', opt)}>{opt === 'NO' ? 'No tiene' : opt}</ChipBtn>)}</div>
          {form.antihurto !== 'NO' && (<div className="grid grid-cols-2 gap-3">{form.antihurto === 'Otro' && <div><Label>Indicar marca</Label><Input value={form.antihurto_otro} onChange={e => set('antihurto_otro', e.target.value)} /></div>}<div><Label>Año instalación</Label><YearSelect value={form.antihurto_year} onChange={e => set('antihurto_year', e.target.value)} /></div></div>)}
        </Section>

        <Section title="Consultoría">
          <div className="flex flex-wrap gap-2">{CONSULTORIA_OPTIONS.map(opt => <ChipBtn key={opt} active={form.consultoria === opt} onClick={() => set('consultoria', opt)}>{opt === 'NO' ? 'No tiene' : opt}</ChipBtn>)}</div>
          {form.consultoria !== 'NO' && (<div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{form.consultoria === 'Otro' && <div className="sm:col-span-3"><Label>Indicar servicio</Label><Input value={form.consultoria_otro} onChange={e => set('consultoria_otro', e.target.value)} /></div>}<div><Label>Mes inicio</Label><Select value={form.consultoria_month} onChange={e => set('consultoria_month', e.target.value)}><option value="">Mes</option>{MONTHS.map((m,i) => <option key={i} value={i+1}>{m}</option>)}</Select></div><div><Label>Año inicio</Label><YearSelect value={form.consultoria_year} onChange={e => set('consultoria_year', e.target.value)} /></div></div>)}
        </Section>

        <Section title="Robot dispensador">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">{ROBOT_OPTIONS.map(opt => <ChipBtn key={opt} active={form.robot === opt} onClick={() => set('robot', opt)}>{opt === 'NO' ? 'No tiene' : opt}</ChipBtn>)}</div>
          {form.robot !== 'NO' && (<div className="grid grid-cols-2 gap-3">{form.robot === 'Otro' && <div><Label>Indicar marca</Label><Input value={form.robot_otro} onChange={e => set('robot_otro', e.target.value)} /></div>}<div><Label>Año instalación</Label><YearSelect value={form.robot_year} onChange={e => set('robot_year', e.target.value)} /></div></div>)}
        </Section>

        <Section title="Cruz luminosa">
          <div className="flex gap-2">{CRUZ_OPTIONS.map(opt => <ChipBtn key={opt} active={form.cruz === opt} onClick={() => set('cruz', opt)}>{opt === 'NO' ? 'No tiene' : opt}</ChipBtn>)}</div>
          {form.cruz !== 'NO' && (<div className="grid grid-cols-2 gap-3"><div><Label>Nº cruces</Label><Input type="number" min="1" value={form.cruz_cantidad} onChange={e => set('cruz_cantidad', e.target.value)} /></div>{form.cruz === 'Puede ampliar' && <div><Label>Nº ampliación prevista</Label><Input type="number" min="1" value={form.cruz_ampliacion} onChange={e => set('cruz_ampliacion', e.target.value)} /></div>}</div>)}
        </Section>

        <Section title="Gestor de turnos">
          <div className="flex gap-2">{['NO','SI'].map(opt => <ChipBtn key={opt} active={form.gestor_turnos === opt} onClick={() => set('gestor_turnos', opt)}>{opt === 'NO' ? 'No tiene' : 'Sí tiene'}</ChipBtn>)}</div>
          {form.gestor_turnos === 'SI' && (<div className="grid grid-cols-2 gap-3"><div><Label>Marca</Label><Input value={form.gestor_turnos_marca} onChange={e => set('gestor_turnos_marca', e.target.value)} /></div><div><Label>Año</Label><YearSelect value={form.gestor_turnos_year} onChange={e => set('gestor_turnos_year', e.target.value)} /></div></div>)}
        </Section>

        <Section title="SPD (Sistema Personalizado de Dosificación)">
          <div className="flex gap-2">{['NO','SI'].map(opt => <ChipBtn key={opt} active={form.spd === opt} onClick={() => set('spd', opt)}>{opt === 'NO' ? 'No tiene' : 'Sí tiene'}</ChipBtn>)}</div>
          {form.spd === 'SI' && (<div className="grid grid-cols-2 gap-3"><div><Label>Marca</Label><Input value={form.spd_marca} onChange={e => set('spd_marca', e.target.value)} /></div><div><Label>Año</Label><YearSelect value={form.spd_year} onChange={e => set('spd_year', e.target.value)} /></div></div>)}
        </Section>

        <Section title="Pantallas">
          <div className="flex gap-2">{['NO','SI'].map(opt => <ChipBtn key={opt} active={form.pantallas === opt} onClick={() => set('pantallas', opt)}>{opt === 'NO' ? 'No tiene' : 'Sí tiene'}</ChipBtn>)}</div>
          {form.pantallas === 'SI' && (<div className="space-y-3"><div className="grid grid-cols-2 gap-3"><div><Label>Marca</Label><Input value={form.pantallas_marca} onChange={e => set('pantallas_marca', e.target.value)} /></div><div><Label>Año</Label><YearSelect value={form.pantallas_year} onChange={e => set('pantallas_year', e.target.value)} /></div></div><div><Label>Ubicación (puede seleccionar varias)</Label><div className="flex gap-5 mt-1">{[['pantallas_interior','Interior'],['pantallas_escaparate','Escaparate'],['pantallas_exterior','Exterior']].map(([field, label]) => (<label key={field} className="flex items-center gap-1.5 text-sm text-gray-700 cursor-pointer"><input type="checkbox" checked={form[field]} onChange={e => set(field, e.target.checked)} className="w-4 h-4 accent-teal-600" />{label}</label>))}</div></div></div>)}
        </Section>

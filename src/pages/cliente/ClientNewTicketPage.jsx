...
export default function ClientNewTicketPage() {
  ...
  const clientProfile = pharmacyId ? { ...profile, pharmacy_id: pharmacyId } : profile
  const { createTicket } = useClientTickets(clientProfile)
  const [form, setForm] = useState({ subject: '', type: 'Incidencia', priority: 'bajo', product: 'Soporte Tecnico - Viteka', description: '' })
  ...
  async function submit(event) {
    ...
    try {
      ...
      navigate(`/cliente/tickets/${ticket.id}`)
    } catch (error) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-5 px-4 py-6 sm:px-6">
      <div>
        <Link to="/cliente/tickets" className="text-xs font-bold text-teal-700 hover:underline">Tickets</Link>
        <h1 className="mt-1 font-display text-2xl font-extrabold text-slate-950">Enviar un ticket</h1>
        <p className="mt-1 text-sm text-slate-500">Cuanta mas informacion incluya, antes podremos ayudarle.</p>
      </div>
      ...
      <form onSubmit={submit} className="card space-y-4 p-5">
        <div>
          ...
          <input readOnly className="field bg-slate-50 text-slate-500" value={profile?.email || ''} />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea rows={7} className="field resize-y" value={form.description} onChange={event => update('description', event.target.value)} placeholder="Describa que ocurre, desde cuando y que estaba intentando hacer." />
        </div>
        <div>
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
          <Link to="/cliente/tickets" className="btn-ghost">Cancelar</Link>
